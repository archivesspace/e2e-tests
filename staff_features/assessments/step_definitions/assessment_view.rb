...
Given 'two Assessments have been created with a common keyword in their record' do
  ...
  visit "#{STAFF_URL}/assessments/new"
  # For some reason, accession cannot be found here, but it shows in the global search results.
  ...
  visit "#{STAFF_URL}/assessments/new"
  fill_in 'token-input-assessment_surveyed_by_', with: "Accession B #{@accession_b_uuid} #{@shared_accession_uuid}"
  dropdown_items = all('li.token-input-dropdown-item2')
  dropdown_items.first.click
...
Given 'the two Assessments are displayed sorted by ascending record' do
  visit "#{STAFF_URL}/assessments"

  fill_in 'filter-text', with: @shared_assessment_uuid
