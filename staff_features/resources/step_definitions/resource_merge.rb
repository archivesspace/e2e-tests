...
Given 'two Resources have been created' do
...
Given 'the first Resource is opened in edit mode' do
...
When 'the user selects the second Resource from the search results in the modal' do
...
When 'the user filters by text with the second Resource title in the modal' do
  within '.modal-content' do
...
When 'the user fills in and selects the second Resource in the merge dropdown form' do
  fill_in 'token-input-merge_ref_', with: "Resource B #{@uuid}"
  dropdown_items = all('li.token-input-dropdown-item2')
  dropdown_items.first.click
end

Then 'the second Resource is deleted' do
  visit "#{STAFF_URL}/resources/#{@resource_second_id}"

  expect(page).to have_text 'Record Not Found'
end

Then 'the following linked records from the second Resource are appended to the first Resource' do |forms|
  visit "#{STAFF_URL}/resources/#{@resource_first_id}/edit"
